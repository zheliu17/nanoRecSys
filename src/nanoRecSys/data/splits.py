...
def create_global_time_split(val_ratio: float = 0.1, test_ratio: float = 0.1):
    ...
    print("Shuffling rows within each split...")
...
def create_user_time_split(val_k: int = 5, test_k: int = 5):
    ...
    """
    print("Loading data for splitting (User-Based)...")
    df = load_processed_data()
    ...
    print("Ranking interactions per user...")
    ...
    print(f"Users in Train: {train_users} (Missing: {n_users - train_users})")
